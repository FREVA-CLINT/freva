...
class Esgf2Solr(object):
   ...
   def find_experiment(self,p2p=P2P()):
       ...
       if len(results['experiment'].keys()) == 0: raise Exception,'Experiment not found'
       self.experiments = results['experiment'].keys()
               
   def get_path(self,p2p=P2P()):
       gzname  = self.outpath+'/solr_crawl_%s.csv.gz' % datetime.now().strftime('%Y-%m-%d_%H%M%S')
       try:
           with gzip.open(gzname,'wb',6) as f:           
               f.write('crawl_dir\t%s%s%s\n' % (self.prepath,self.prefix,self.project['project']))
               f.write('\n')
               f.write('data\n')
               for experiment in self.experiments:
                   if self.experiment <> None and experiment <> self.experiment: continue
                   facets = self.facets
                   facets.update({'experiment':experiment})
                   esgffiles={}
                   for dataset in p2p.get_datasets(fields=','.join(self.fields),**facets):
                       ...
                       esgfpath += '/'
                       timestamp = time.mktime((datetime.strptime(dataset['timestamp'],'%Y-%m-%dT%H:%M:%SZ')).timetuple())
                       if self.prepath+esgfpath+filename not in esgffiles: esgffiles[self.prepath+esgfpath+filename] = 0
                       if timestamp >= esgffiles[self.prepath+esgfpath+filename]: esgffiles[self.prepath+esgfpath+filename]=timestamp
