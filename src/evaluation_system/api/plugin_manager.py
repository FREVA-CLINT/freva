...
"""
    ...
    if user: user.getUserDB().storeHistory(p, complete_conf, user.getUserName(), 0, result=result)
...
See :class:`evaluation_system.model.db.UserDB.getHistory` for more information on this interface."""
    ...
    return user.getUserDB().getHistory(plugin_name, limit, since=since, until=until, entry_ids=entry_ids, uid = user.getUserName())
