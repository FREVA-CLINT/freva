...
def testStartSlurm(dummy_config):
    ...
    slurm_in_dir = os.path.join(
        test_user.getUserSchedulerInputDir(), test_user.getName()
    )
    ...
    with open(infile, "w") as fp:
        sf = slurm.slurm_file()
        sf.set_default_options(test_user, "cat $0", outdir=slurm_out_dir)
        sf.write_to_file(fp)
        fp.flush()
    ...
    assert out_first_line.split(" ")[0] == "Submitted", "Received: " + str(
        out_first_line
    )
