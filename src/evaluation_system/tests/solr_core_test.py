...
def test_ingest(dummy_solr):
      ...
      # add new version
      new_version = Path(dummy_solr.tmpdir) / 'cmip5/output1/MOHC/HadCM3/decadal2009/mon/atmos/Amon/r7i2p1/v20120419/ua/ua_Amon_HadCM3_decadal2009_r7i2p1_200911-201912.nc'
      new_version.parent.mkdir(parents=True, exist_ok=True)
      new_version.touch()
      SolrCore.load_fs(new_version, abort_on_errors=True,
                       core_all_files=dummy_solr.all_files,
                       core_latest=dummy_solr.latest)
      assert set(ff_all._search()).symmetric_difference(set(all_entries)).pop() == str(new_version)
