    ...
    for case_type, positive_cases, negative_cases in test_cases:
        for expected, result in positive_cases:
            ...
            if isinstance(parsed_value, list):
                assert type(parsed_value[0]) == \
                                  case_type.base_type
            else:
                assert type(parsed_value) == \
                                  case_type.base_type
...
def test_parse_arguments(dummy_env):
    ...
    assert res == dict(int=1, date='1', bool=True,
                               range=list(range(1,6)), file='/tmp/file1',
    ...
    for arg, parsed_val in [("bool=1",True), ("bool=true",True),
        ...
        assert res == {arg.split('=')[0]:parsed_val}, \
    ...
    assert p_dict.parseArguments(["file=a:b"]) == \
                      dict(file=['a','b'])
    with pytest.raises(ValidationError):
    ...
    assert p_dict.parseArguments(["date=a/b"]) == \
                      dict(date="a/b")
