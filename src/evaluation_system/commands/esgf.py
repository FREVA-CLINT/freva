...
class Command(FrevaBaseCommand):
    ...
    _short_args = 'hd'
#    _args = ['facet=','query=', 'datasets', 'show-facet=',
#             'opendap', 'gridftp', 'download-script=', 'debug', 'help']
    _args = [
             ...
             {'name':'--datasets','help':'List the name of the datasets instead of showing the urls.','action':'store_true'},
             {'name':'--show-facet','help':'<list> List all values for the given facet (might be defined multiple times). The results show the possible values of the selected facet according to the given constraints and the number of *datasets* (not files) that selecting such value as a constraint will result (faceted search)','metavar':'FACET'},
             {'name':'--opendap','help':'List the name of the datasets instead of showing the urls.','action':'store_true'},
             {'name':'--gridftp','help':'Show Opendap endpoints instead of the http default ones (or skip them if none found)','action':'store_true'},
             {'name':'--download-script','help':'<file> Download wget_script for getting the files instead of displaying anything (only http)','metavar':'FILE'},
...
"""
    ...
    def _run(self):
        ...
        for arg in lastargs:
            items = arg.split('=')
            if items[0] not in facets: facets[items[0]] = [] 
            if len(items)>1:
