...
of the DB considerably without the risk of loosing information.'''
    ...
    def safeExecute(self, *args, **kwargs):
        ...
        try:
            ret = self._getConnection().execute(*args, **kwargs)
    ...
    def safeExecutemany(self, *args, **kwargs):
        ...
        try:
            ret = self._getConnection().executemany(*args, **kwargs)
        except (AttributeError, MySQLdb.OperationalError):
            _connection_pool.pop(self._db_file, None)
            ret = self._getConnection().executemany(*args, **kwargs)
...
:param tool_name: name of the tool whose DB/table will get initialized. We are not using
        ...
        if not self.isInitialized():
            ...
            for table_name, version in self.__tables['__order']:
                ...
                try:
                    res = self.safeExecute('SELECT * FROM meta WHERE table_name = %s AND version = %s', (table_name, version))
                    res = res.fetchone();
                    if res:
...
#                json.dumps(result),
        ...
        self.safeExecute("""INSERT INTO history_history(timestamp,tool,version,configuration,slurm_output,uid,status) VALUES(%s, %s, %s, %s, %s, %s, %s);""", row)
    ...
    def upgradeStatus(self, row_id, uid, status):
	    ...
	    self.safeExecute(select_str, (row_id,uid))
