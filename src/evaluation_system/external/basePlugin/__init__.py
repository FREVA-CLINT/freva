...
def apply_workarounds_for_path(filelist):
    """
    some decadal experiments are not located in the same location then other experiments that belong together. the result ist that 
    ...
    for i in range(len(filelist)):
        ...
        if has_multiple_roots:
            match = re.match("/miklip/integration/data4miklip/projectdata/baseline0/output1/MPI-M/MPI-ESM-LR/decadal(\d\d\d\d)/(day|mon|6hr)/atmos/"+filelist[i].dict["parts"]["variable"]+"/"+filelist[i].dict["parts"]["ensemble"], filelist[i].to_path())
            if match is not None:
                newfile = splittedFile([filelist[i]])
                replacement = "/miklip/integration/data4miklip/model/global/miklip/baseline0/output1/MPI-M/MPI-ESM-LR/decadal%s/%s/atmos/%s/%s/v20111122/%s" % (match.group(1), match.group(2), filelist[i].dict["parts"]["cmor_table"], filelist[i].dict["parts"]["ensemble"], filelist[i].dict["parts"]["variable"])
...
def get_time_format_and_start_and_end(name_or_file, allow_no_time=False):
    ...
    if isinstance(name_or_file, splittedFile) or isinstance(name_or_file, DRSFile):
        ...
        if re.match("\d{6,12}-\d{6,12}", timepart) == None:
...
def get_start_and_end_time_from_DRSFile(dfile, include_str=True, only_str=False):
    ...
    try:
        ...
        Logger.Error("The filename %s contains an invalid date: %s" % (dfile.to_path(), ve.message), -1)
    if include_str == True and only_str == False:
...
def solr_search_multivar(variables, ssargs):
    ...
    for onefile in allfiles:
        if os.path.exists(onefile.to_path()) and not onefile.to_path().endswith(".nc.save") and not os.path.getsize(onefile.to_path()) == 0:
...
class splittedFile(object):
    '''
    Files of the same experiment are sometimes splitted up if they would become to large otherwise. These splitted files
    ...
    def to_path(self, fake=False, as_list=False, separator=" "):
        ...
        if fake == True:
            ...
            # get the first start time and the last end time
            return get_path_string(self.parts[0], pfake=True).replace(self.parts[0].dict["parts"]["time"], self.dict["parts"]["time"]).replace(self.parts[0].dict["parts"]["variable"], self.dict["parts"]["variable"])
        else:
            if as_list:
                result = []
                for onefile in self.parts:
...
class basePlugin(object):
    ...
    def search_files(self, decadals=None, project="baseline1", firstyear=None, lastyear=None, product="*", time_frequency='6hr', model="mpi-esm-lr", 
        ...
        set find_variables=True to get a list of available variables instead of seraching files
        '''
...
#                          'Please select one',-1)
        ...
        if decadals != None:
            ...
            years = None
        if firstyear != None: firstyear = int(firstyear)
        if lastyear  != None: lastyear  = int(lastyear)
        
        # use solr_search to get the input files --------------------------------------------------
        # construct a search string for experiments
        experiment_prefix = experiment
        if experiment_prefix == None or experiment_prefix == "*":
        ...
        self.inputfiles = []
        if years != None or firstyear!= None or lastyear!= None:
            self.inputfilesByDecade = {}
            # we have multiple experiments that contain the decade
            if "experiment" in ssargs and project != "observations" and project != "reanalysis" and years!=None:
                for year in years:
                    yearfiles = []
                    ...
                    for onefile in solr_search_multivar(variables, ssargs):
                        ...
                        for year in years:
                            if starttime.year > year and endtime.year <= year+10:
                                self.inputfiles.append(onefile)
                                if not year in self.inputfilesByDecade:
        ...
        # check for overlapping timeperiods within the same folder
    ...
    def group_input_by(self, files, groupby="ensemble", groupby2=None):
        ...
        def get_dataset_without_groupby(dfile):
            ...
            if groupby2 != None and groupby2 in dfile.dict["parts"]:
        ...
        if not groupby in files[0].dict["parts"]:
            return None

        # put all file in a dictionary where the key is the dataset. file that belong to the same dataset are placed in lists
        datasets = {}
    ...
    def check_ensemble_completeness(self, files):
        ...
        if not "ensemble" in files[0].dict["parts"]:
            return files
        ...
        for onefile in files:
            ...
            if not ensemble_part in ensembles:
    ...
    def remove_overlapping_time_periods_from_file_list(self, filelist):
        """
        this is a workaround only. Removed are files that are located in the same folder but have overlapping time periods 
        """
        ...
        for var in variables:
            ...
            for onefile in files:
                if onefile.dict["parts"]["variable"] == var:
                    ...
                    if not time_part in all_times:
                        all_times.append(time_part)
                    if not ensemble_part in all_ensemble:
    ...
    def filter_inputfiles_by_lead_year(self, filelist, lead_year):
        """
        files those are created by split_file_list_by_timecontinously and those contain single files only one year long 
    ...
    def calculate_ensemble_stat(self, plugin_path, dryrun=False, separate=True, has_mon=False):
        ...
        @separate   set to True if you don't want to add the files to the list of output files. The additional list outputfiles_ensstat only will be filled.
        '''
        if self.ensemble_groups == None:
        ...
        for key, value in self.ensemble_groups.iteritems():
            group_files = []
            for stat in stats:
                output_name = self.get_output_from_input_name(key.replace("<ensemble>", stat).replace("<rcm_ensemble>", stat))
                ...
                if has_mon == True:
                    ...
                    for ifile in value:
                        if isinstance(ifile, splittedFile):
                            command2.addPositionalArgument(self.get_output_from_input_name(ifile.to_path(fake=True)).replace(".nc", "_mon.nc"))
                        else:
                            command2.addPositionalArgument(self.get_output_from_input_name(ifile.to_path()).replace(".nc", "_mon.nc"))
